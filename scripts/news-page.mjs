...
function visibleItems(data) {
  return data.items.filter(item => {
    ...
    return Number.isFinite(age) && age >= -300000 && age <= (item.category === 'AI' ? 14 : 3) * 86400000;
  });
}
function view(data, category = '速览', topic = '全部') {
  const e = escapeHtml, all = visibleItems(data);
  let items = all.filter(x => x.category === category && (topic === '全部' || x.topic === topic));
  if (category === '速览') {
    items = [];
    // Balanced reading sample, publication-time order within each category; no popularity score.
    for (const name of Object.keys(TOPICS)) {
      const pool = all.filter(x => x.category === name).sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt));
      const topics = new Set();
      for (const item of pool) { if (topics.has(item.topic)) continue; topics.add(item.topic); items.push(item); if (topics.size === 3) break; }
    }
  }
  const tabs = ['速览', ...Object.keys(TOPICS)].map(name => `<button type="button" class="tab ${name === category ? 'active' : ''}" data-category="${name}" aria-pressed="${name === category}">${name}${name === '速览' ? '' : `<span>${all.filter(x => x.category === name).length}</span>`}</button>`).join('');
  const subnav = category === '速览' ? '<p class="subhint">各栏目摘选 · 同类按发布时间排列 · 编号仅用于索引</p>' : ['全部', ...TOPICS[category]].map(name => `<button type="button" class="chip-button ${name === topic ? 'active' : ''}" data-topic="${e(name)}" aria-pressed="${name === topic}">${e(name)}${name === '全部' ? '' : ` <span>${all.filter(x => x.category === category && x.topic === name).length}</span>`}</button>`).join('');
  const card = item => {
    ...
    return `<article class="news-card"><div class="card-kicker"><span class="number">NO. ${String(item.number).padStart(2, '0')}</span><span>${e(item.category)} / ${e(item.topic)}</span>${stale ? '<span class="warning">暂用缓存</span>' : ''}</div><h3><a href="${e(safeUrl(item.url))}" target="_blank" rel="noopener noreferrer">${e(item.title)}</a></h3><p class="summary">${e(shorten(item.summary))}</p><div class="card-meta"><span class="source-chip">${e(item.source)}</span><span>${e(time)}</span></div>${item.reportDate && item.reportDate !== dateKey() ? `<p class="older">AI 最近一期：${e(item.reportDate.replace(/-/g, '/'))}，今日版待更新</p>` : ''}<a class="original" href="${e(safeUrl(item.url))}" target="_blank" rel="noopener noreferrer">阅读原文 <span aria-hidden="true">↗</span></a></article>`;
  };
  const groups = category === '速览' ? Object.keys(TOPICS) : topic === '全部' ? TOPICS[category] : [topic];
  const content = groups.map((group, i) => {
    const rows = items.filter(x => category === '速览' ? x.category === group : x.topic === group);
    return `<section class="news-section" id="group-${i}" aria-labelledby="heading-${i}"><div class="section-heading"><h2 id="heading-${i}">${e(group)}</h2><span>${rows.length} 条${category === '速览' ? '摘选' : ''}</span>${category === '速览' ? `<button class="text-button" type="button" data-category="${e(group)}">查看全部 →</button>` : ''}</div>${rows.length ? `<div class="news-grid">${rows.map(card).join('')}</div>` : `<p class="empty">${category === 'AI' ? '最近一期暂无此类条目。' : '近 72 小时暂未收录符合范围的报道。'}${data.sources.some(s => s.status === 'error') ? ' 部分来源暂时不可用，可展开页尾查看。' : ''}</p>`}</section>`;
  }).join('');
...
function browserApp() {
  ...
  let category = '速览', topic = '全部', busy = false, lastChecked = 0;
  const $ = id => document.getElementById(id);
  function render() {
    const result = view(data, category, topic);
    ...
    $('freshness').textContent = stale ? '已发布内容超过 6 小时未更新。定时发布可能延迟，当前保留最近内容。' : '体育、金融、政治收录近 72 小时内容；AI 显示最近一期。时间均为北京时间。';
    $('freshness').classList.toggle('warning', stale);
  }
  ...
  async function refresh(manual = false) {
    ...
    try {
      ...
      const seen = new Set();
      data.items = data.items.filter(x => { const key = x.url.split(/[?#]/)[0]; if (seen.has(key)) return false; seen.add(key); return true; });
...
export function buildHomepage(data) {
  ...
  const functions = [clean, shorten, safeUrl, dateKey, escapeHtml, humanTime, visibleItems, view, browserApp].map(fn => {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#176448"><meta name="apple-mobile-web-app-title" content="每日简报"><meta name="description" content="面向手机阅读的个人每日简报：NBA、五大联赛、网球、美股、A股、中国期货、中美政治与AI。"><title>每日简报 · 体育 / 金融 / 政治 / AI</title><style>${CSS}</style></head><body id="top"><header class="masthead"><div class="shell"><div class="brand-row"><div><p class="eyebrow">DAILY BRIEF / 每日阅读</p><h1>你的每日简报</h1></div><span class="edition">手机试读版</span></div><p class="date-line"><span id="today">${escapeHtml(new Date().toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai', year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' }))}</span> · 收录 <strong id="total">${first.count}</strong> 条</p><div class="sync"><div class="sync-copy"><p>新闻采集于 <span id="collected">${humanTime(data.collectedAt)}</span></p><p id="status" role="status" aria-live="polite">已加载可阅读快照；打开后自动检查更新。</p></div><button type="button" id="refresh">检查更新</button></div></div></header><nav class="navigation" id="navigation" aria-label="新闻栏目"><div class="shell"><div class="tabs" id="tabs">${first.tabs}</div><div class="subnav" id="subnav">${first.subnav}</div></div></nav><main class="shell"><p id="freshness" class="freshness">体育、金融、政治收录近 72 小时内容；AI 显示最近一期。时间均为北京时间。</p><noscript>已展示各栏目摘选。启用 JavaScript 后可筛选全部新闻和检查更新。</noscript><div id="content">${first.content}</div></main><footer><div class="shell"><p>本页共 <strong id="footer-total">${first.count}</strong> 条 · 数据来源：网易体育、新浪财经、中国新闻网、AI HOT。</p><p>热点由主题筛选和时间排序产生，不代表全网热度排名。摘要为来源摘录，观点与未证实说法请结合署名及原文阅读。</p><p>体育、金融、政治由云端约每 2 小时尝试更新，平台调度可能延迟；“检查更新”读取已发布内容。AI 额外直连最新日报。市场休市时仍可阅读最近报道。</p><div class="footer-links"><a href="ai.html">AI 完整日报</a><a href="archive/">AI 历史归档</a><a href="#top">返回顶部 ↑</a></div><details><summary>查看数据来源与更新状态</summary><ul id="source-list">${data.sources.map(s => `<li><a href="${escapeHtml(s.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(s.name)}</a><span>${s.status === 'error' ? '采集失败 · ' : ''}最近成功：${humanTime(s.lastSuccess)}</span></li>`).join('')}</ul></details></div></footer><script type="application/json" id="initial-data">${serialized}</script><script>const TOPICS=${JSON.stringify(TOPICS)};\n${functions}\nbrowserApp();</script></body></html>`;
}
